...
class LargeBeneficiaryExtractor @Inject() extends PlaybackExtractor[Option[List[DisplayTrustLargeType]]] with Logging {
  ...
  private def extractNumberOfBeneficiaries(numberOfBeneficiary: String, index: Int, answers: UserAnswers): Try[UserAnswers] = {
    numberOfBeneficiary match {
      case "over1" => answers.set(LargeBeneficiaryNumberOfBeneficiariesPage(index), Over1)
      case "over101" => answers.set(LargeBeneficiaryNumberOfBeneficiariesPage(index), Over101)
      case "over201" => answers.set(LargeBeneficiaryNumberOfBeneficiariesPage(index), Over201)
      case "over501" => answers.set(LargeBeneficiaryNumberOfBeneficiariesPage(index), Over501)
